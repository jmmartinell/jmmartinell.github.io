(function () {
    // Categorías con estructura de árbol
    const CATEGORIES = [
        // --- RAÍZ ---
    ...
    App.Data.Catalog = {
        ...
        getProductById: (id) => CATALOG.find(p => p.id === id) || null,
        getDescendantCategoryIds: function (categoryId) {
            if (categoryId === 'root') {
            ...
            for (const child of children) {
                ids = ids.concat(this.getDescendantCategoryIds(child.id));
            }
            return ids;
        },
        getRootCategories: () => CATEGORIES.filter(c => c.parentId === null),
        getSubcategories: (parentId) => CATEGORIES.filter(c => c.parentId === parentId)
    };
})();
